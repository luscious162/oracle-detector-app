...
# --- 配置常量 (与 prediction_logic.py 保持一致或根据需要调整) ---
...
DEFAULT_CLOSING_KERNEL_SIZE = 5 # 与 prediction_logic.py 中的值保持一致
DEFAULT_CLOSING_ITERATIONS = 2  # 与 prediction_logic.py 中的值保持一致
DEFAULT_OPENING_KERNEL_SIZE = 3 # 与 prediction_logic.py 中的值保持一致
DEFAULT_OPENING_ITERATIONS = 1  # 与 prediction_logic.py 中的值保持一致
DEFAULT_MIN_AREA = 4000         # 与 prediction_logic.py 中的值保持一致
DEFAULT_MAX_AREA_THRESHOLD = 10000000 # 与 prediction_logic.py 中的值保持一致
DEFAULT_MAX_LETTER_HEIGHT = 50  # 与 prediction_logic.py 中的值保持一致
DEFAULT_MAX_LETTER_WIDTH = 50   # 与 prediction_logic.py 中的值保持一致
DEFAULT_BORDER_MARGIN = 15      # 与 prediction_logic.py 中的值保持一致
DEFAULT_MAX_RELATIVE_SIZE = 0.9 # 与 prediction_logic.py 中的值保持一致
DEFAULT_MIN_CONFIDENCE_THRESHOLD = 0.5 # 与 prediction_logic.py 中的值保持一致
DEFAULT_PATCH_HEIGHT = 160 # 需要与 TFLite 模型输入尺寸一致
DEFAULT_PATCH_WIDTH = 160  # 需要与 TFLite 模型输入尺寸一致
DEFAULT_CONTOUR_THICKNESS = 3   # 与 prediction_logic.py 中的值保持一致

# --- TFLite 模型加载函数 (与 prediction_logic.py 中相同) ---
...
# --- 分割函数 (与 prediction_logic.py 中相同) ---
def find_rubbings_and_calculate_area_for_prediction(image_path,
    ...
    # ... (分割函数的代码与 prediction_logic.py 中完全相同) ...
    # --- 查找轮廓 ---
    print("  [分割函数] 查找轮廓...")
    try:
...
# 函数名保持 predict_rubbings 以便 app.py 调用
def predict_rubbings(image_path,
    ...
    """
    print(f"\n--- [核心预测 TFLite - batch_test] 开始处理图像: {image_path} ---") # 添加标识
    ...
    try:
        ...
        if not rubbing_infos:
            print("  [核心预测 TFLite - batch_test] 未找到符合条件的轮廓。")
    ...
    print(f"  [核心预测 TFLite - batch_test] 找到 {len(rubbing_infos)} 个候选拓片进行分类。")
    ...
    for i, info in enumerate(rubbing_infos):
        ...
        try:
            ...
            # !! 关键：这里的预处理必须匹配模型 !!
            patch_processed = patch_resized.astype(input_dtype)
            patch_processed = (patch_processed / 127.5) - 1.0 # 假设归一化到 [-1, 1]
            patch_batch = np.expand_dims(patch_processed, axis=0)

            # --- TFLite 预测 ---
            print(f"    预测中 (TFLite - batch_test)...")
            interpreter.set_tensor(input_index, patch_batch)
            interpreter.invoke()
    ...
    print(f"--- [核心预测 TFLite - batch_test] 处理完成. 保留了 {kept_predictions_count} 个预测。 ---")
    return predictions_api_list, result_image_final, None


# --- 绘制结果函数 (与 prediction_logic.py 中相同) ---
...
# --- 主预测流程 (用于独立测试 predict_batch_test.py) ---
if __name__ == "__main__":
    print("--- 独立脚本测试流程开始 (TFLite - batch_test) ---")
    # 使用文件顶部的默认常量进行测试
    test_image = r"E:/oracle_talent/oracle_data/tokyo_data/27-41.jpg" # 或者选择一个实际存在的测试图片
    test_output_dir = r"./test_output_tflite_batch" # 保存到不同的目录
    ...
    if err_msg:
        ...
        if result_img is not None:
            try:
                output_path = os.path.join(test_output_dir, "tflite_batch_test_result_" + os.path.basename(test_image))
